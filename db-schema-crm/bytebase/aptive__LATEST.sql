...
CREATE SCHEMA billing_old;

CREATE SCHEMA crm;

CREATE SCHEMA customer;

CREATE SCHEMA customer_old;
...
CREATE SCHEMA field_operations_old;
...
CREATE SCHEMA organization_old;

CREATE SCHEMA pestroutes;

CREATE SCHEMA product;

CREATE SCHEMA product_old;

CREATE SCHEMA sales;

CREATE SCHEMA sales_old;
...
BEGIN
        ...
        IF OLD is distinct from NEW AND TG_OP != 'DELETE' THEN
            ...
            join jsonb_each(to_jsonb(new)) as _new(key,value) on (_old.key = _new.key) and _old.key != 'pestroutes_json'
            ...
            ',TG_TABLE_SCHEMA, TG_TABLE_NAME) USING NEW.id, _operation, OLD, NEW, _json_diff, current_user, (SELECT inet_client_addr()), 'PENDING';
            RETURN NEW;
        ...
        ElSIF TG_OP = 'DELETE' THEN
            EXECUTE format('
                ...
                VALUES ( $1, $2, $3, row_to_json($4), $5, $6)
            ',TG_TABLE_SCHEMA, TG_TABLE_NAME) USING OLD.id, 'HARD_DELETE', OLD, current_user, (SELECT inet_client_addr()), 'PENDING';
            RETURN OLD;
...
BEGIN
    ...
    SELECT object_identity INTO _table_name
    FROM pg_event_trigger_ddl_commands()
    WHERE command_tag = 'CREATE TABLE';

    -- If the event is for a new or altered table
    IF _table_name IS NOT NULL THEN
        ...
        EXECUTE 'ALTER TABLE ' || _table_name || ' DROP COLUMN IF EXISTS created_by';
        EXECUTE 'ALTER TABLE ' || _table_name || ' DROP COLUMN IF EXISTS updated_by';
        EXECUTE 'ALTER TABLE ' || _table_name || ' DROP COLUMN IF EXISTS deleted_by';
        EXECUTE 'ALTER TABLE ' || _table_name || ' DROP COLUMN IF EXISTS created_at';
        EXECUTE 'ALTER TABLE ' || _table_name || ' DROP COLUMN IF EXISTS updated_at';
        EXECUTE 'ALTER TABLE ' || _table_name || ' DROP COLUMN IF EXISTS deleted_at';

        EXECUTE 'ALTER TABLE ' || _table_name || ' ADD COLUMN IF NOT EXISTS created_by urn';

        EXECUTE 'ALTER TABLE ' || _table_name || ' ADD COLUMN IF NOT EXISTS updated_by urn';

        EXECUTE 'ALTER TABLE ' || _table_name || ' ADD COLUMN IF NOT EXISTS deleted_by urn';

        EXECUTE 'ALTER TABLE ' || _table_name || ' ADD COLUMN IF NOT EXISTS created_at TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT NOW()';

        EXECUTE 'ALTER TABLE ' || _table_name || ' ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT NOW()';

        EXECUTE 'ALTER TABLE ' || _table_name || ' ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP(6) WITH TIME ZONE';
    END IF;
END;
$$;
...
CREATE TABLE audit.customer__customers (
...
ALTER TABLE audit.customer__subscriptions ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME audit.customer__subscriptions_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE auth_old.actions ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME auth_old.actions_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE auth_old.fields ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME auth_old.fields_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE auth_old.idp_roles (
    id integer NOT NULL,
    role_id integer NOT NULL,
...
ALTER TABLE auth_old.idp_roles ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME auth_old.idp_roles_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE auth_old.permissions (
    id integer NOT NULL,
    service_id integer NOT NULL,
    resource_id integer NOT NULL,
    field_id integer NOT NULL,
...
ALTER TABLE auth_old.permissions ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME auth_old.permissions_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE auth_old.resources (
    id integer NOT NULL,
    name text NOT NULL,
    created_at timestamp(6) without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp(6) without time zone DEFAULT CURRENT_TIMESTAMP,
...
ALTER TABLE auth_old.resources ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME auth_old.resources_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE auth_old.role_permissions (
    id integer NOT NULL,
    role_id integer NOT NULL,
    permission_id integer NOT NULL,
    created_at timestamp(6) without time zone DEFAULT CURRENT_TIMESTAMP,
...
ALTER TABLE auth_old.role_permissions ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME auth_old.role_permissions_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE auth_old.roles ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME auth_old.roles_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE auth_old.services (
    id integer NOT NULL,
    name text NOT NULL,
    created_at timestamp(6) without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp(6) without time zone DEFAULT CURRENT_TIMESTAMP,
...
ALTER TABLE auth_old.services ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME auth_old.services_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE auth_old.user_roles (
    id integer NOT NULL,
    user_id uuid NOT NULL,
    role_id integer NOT NULL,
    created_at timestamp(6) without time zone DEFAULT CURRENT_TIMESTAMP,
...
ALTER TABLE auth_old.user_roles ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME auth_old.user_roles_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE billing.payment_methods (
    ...
    cc_expiration_month character(2),
    cc_expiration_year character(4),
...
ALTER TABLE billing.transaction_types ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
...
CREATE TABLE billing_old.invoices (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    external_ref_id integer,
    area_id integer,
    customer_id uuid,
    ...
    pestroutes_service_type_id integer,
    pestroutes_created_by integer,
    pestroutes_created_at timestamp with time zone,
    pestroutes_invoiced_at timestamp with time zone,
    pestroutes_updated_at timestamp with time zone,
    pestroutes_office_id integer
);

COMMENT ON COLUMN billing_old.invoices.external_ref_id IS '{"pestroutes_column_name": "ticketID"}';

COMMENT ON COLUMN billing_old.invoices.is_active IS '{"pestroutes_column_name": "active"}';

COMMENT ON COLUMN billing_old.invoices.subtotal IS '{"pestroutes_column_name": "subTotal"}';

COMMENT ON COLUMN billing_old.invoices.tax_rate IS '{"pestroutes_column_name": "taxRate"}';

COMMENT ON COLUMN billing_old.invoices.total IS '{"pestroutes_column_name": "total"}';

COMMENT ON COLUMN billing_old.invoices.balance IS '{"pestroutes_column_name": "balance"}';

COMMENT ON COLUMN billing_old.invoices.service_charge IS '{"pestroutes_column_name": "serviceCharge"}';

COMMENT ON COLUMN billing_old.invoices.pestroutes_customer_id IS '{"pestroutes_column_name": "customerID"}';

COMMENT ON COLUMN billing_old.invoices.pestroutes_subscription_id IS '{"pestroutes_column_name": "subscriptionID"}';

COMMENT ON COLUMN billing_old.invoices.pestroutes_service_type_id IS '{"pestroutes_column_name": "serviceID"}';

COMMENT ON COLUMN billing_old.invoices.pestroutes_created_by IS '{"pestroutes_column_name": "createdBy"}';

COMMENT ON COLUMN billing_old.invoices.pestroutes_created_at IS '{"pestroutes_column_name": "dateCreated"}';

COMMENT ON COLUMN billing_old.invoices.pestroutes_invoiced_at IS '{"pestroutes_column_name": "invoiceDate"}';

COMMENT ON COLUMN billing_old.invoices.pestroutes_updated_at IS '{"pestroutes_column_name": "dateUpdated"}';

CREATE TABLE billing_old.payment_gateways (
    id integer NOT NULL,
    name character varying(255) NOT NULL,
...
ALTER TABLE billing_old.payment_gateways ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME billing_old.payment_gateways_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE billing_old.payment_invoice_allocations (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    payment_id uuid NOT NULL,
    invoice_id uuid NOT NULL,
    applied_at timestamp(6) with time zone NOT NULL,
...
CREATE TABLE billing_old.payment_methods (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    external_ref_id integer,
    area_id integer,
    ...
    payment_gateway_id integer NOT NULL,
    payment_type_id integer NOT NULL,
    name_on_account_first character varying(255) NOT NULL,
    name_on_account_last character varying(255) NOT NULL,
    description character varying(255),
    ach_account_number_encrypted character varying(255),
    ach_routing_number character(9),
    ach_account_type_id character varying(255),
    cc_token character varying(255),
    ...
    pestroutes_created_at timestamp(0) with time zone,
    pestroutes_updated_at timestamp(0) with time zone,
    is_pestroutes_auto_pay_enabled boolean DEFAULT false NOT NULL,
    created_by uuid,
    updated_by uuid,
    deleted_by uuid,
    created_at timestamp(6) with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
CREATE TABLE billing_old.payment_statuses (
    id integer NOT NULL,
    external_ref_id integer,
    name character varying(255) NOT NULL,
    description character varying(255),
...
ALTER TABLE billing_old.payment_statuses ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME billing_old.payment_statuses_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE billing_old.payment_types (
    id integer NOT NULL,
    name character varying(255) NOT NULL,
    description character varying(255),
    is_hidden boolean DEFAULT false NOT NULL,
...
ALTER TABLE billing_old.payment_types ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME billing_old.payment_types_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE billing_old.payments (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    external_ref_id integer,
    area_id integer,
    customer_id uuid,
...
COMMENT ON COLUMN billing_old.payments.external_ref_id IS '{"pestroutes_column_name": "paymentID"}';

COMMENT ON COLUMN billing_old.payments.amount IS '{"pestroutes_column_name": "amount"}';

COMMENT ON COLUMN billing_old.payments.applied_amount IS '{"pestroutes_column_name": "appliedAmount"}';

COMMENT ON COLUMN billing_old.payments.notes IS '{"pestroutes_column_name": "notes"}';

COMMENT ON COLUMN billing_old.payments.is_office_payment IS '{"pestroutes_column_name": "officePayment"}';

COMMENT ON COLUMN billing_old.payments.is_collection_payment IS '{"pestroutes_column_name": "collectionPayment"}';

COMMENT ON COLUMN billing_old.payments.is_write_off IS '{"pestroutes_column_name": "writeOff"}';

COMMENT ON COLUMN billing_old.payments.pestroutes_office_id IS '{"pestroutes_column_name": "officeID"}';

COMMENT ON COLUMN billing_old.payments.pestroutes_customer_id IS '{"pestroutes_column_name": "customerID"}';

COMMENT ON COLUMN billing_old.payments.pestroutes_created_by IS '{"pestroutes_column_name": "employeeID"}';

COMMENT ON COLUMN billing_old.payments.pestroutes_created_at IS '{"pestroutes_column_name": "date"}';

COMMENT ON COLUMN billing_old.payments.pestroutes_updated_at IS '{"pestroutes_column_name": "dateUpdated"}';
...
ALTER TABLE crm.generic_flags ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
...
COMMENT ON COLUMN customer.addresses.state IS '{"pestroutes_column_name": "customerID"}';
...
CREATE TABLE customer_old.cancellation_reasons (
    id integer NOT NULL,
    area_id integer,
    external_ref_id integer,
    name text NOT NULL,
...
ALTER TABLE customer_old.cancellation_reasons ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME customer_old.cancellation_reasons_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE customer_old.customers (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    external_ref_id integer,
    area_id integer,
    first_name character varying(255),
    ...
    billing_postal_code character varying(24),
    billing_phone character varying(24),
    ...
    pestroutes_customer_link character varying(255),
    pestroutes_created_at timestamp with time zone,
    pestroutes_cancelled_at timestamp with time zone,
    pestroutes_updated_at timestamp with time zone,
    created_by uuid,
    updated_by uuid,
    deleted_by uuid,
...
COMMENT ON COLUMN customer_old.customers.id IS '{"pestroutes_column_name": "customerLink"}';

COMMENT ON COLUMN customer_old.customers.external_ref_id IS '{"pestroutes_column_name": "customerID"}';

COMMENT ON COLUMN customer_old.customers.first_name IS '{"pestroutes_column_name": "fname"}';

COMMENT ON COLUMN customer_old.customers.last_name IS '{"pestroutes_column_name": "lname"}';

COMMENT ON COLUMN customer_old.customers.company_name IS '{"pestroutes_column_name": "companyName"}';

COMMENT ON COLUMN customer_old.customers.is_active IS '{"pestroutes_column_name": "status"}';

COMMENT ON COLUMN customer_old.customers.email IS '{"pestroutes_column_name": "email"}';

COMMENT ON COLUMN customer_old.customers.phone1 IS '{"pestroutes_column_name": "phone1"}';

COMMENT ON COLUMN customer_old.customers.phone2 IS '{"pestroutes_column_name": "phone2"}';

COMMENT ON COLUMN customer_old.customers.address IS '{"pestroutes_column_name": "address"}';

COMMENT ON COLUMN customer_old.customers.city IS '{"pestroutes_column_name": "city"}';

COMMENT ON COLUMN customer_old.customers.state IS '{"pestroutes_column_name": "state"}';

COMMENT ON COLUMN customer_old.customers.postal_code IS '{"pestroutes_column_name": "zip"}';

COMMENT ON COLUMN customer_old.customers.latitude IS '{"pestroutes_column_name": "lat"}';

COMMENT ON COLUMN customer_old.customers.longitude IS '{"pestroutes_column_name": "lng"}';

COMMENT ON COLUMN customer_old.customers.source IS '{"pestroutes_column_name": "source"}';

COMMENT ON COLUMN customer_old.customers.autopay_type IS '{"pestroutes_column_name": "aPay"}';

COMMENT ON COLUMN customer_old.customers.paid_in_full IS '{"pestroutes_column_name": "paidInFull"}';

COMMENT ON COLUMN customer_old.customers.balance IS '{"pestroutes_column_name": "balance"}';

COMMENT ON COLUMN customer_old.customers.balance_age IS '{"pestroutes_column_name": "balanceAge"}';

COMMENT ON COLUMN customer_old.customers.responsible_balance IS '{"pestroutes_column_name": "responsibleBalance"}';

COMMENT ON COLUMN customer_old.customers.responsible_balance_age IS '{"pestroutes_column_name": "responsibleBalanceAge"}';

COMMENT ON COLUMN customer_old.customers.pestroutes_master_account IS '{"pestroutes_column_name": "masterAccount"}';

COMMENT ON COLUMN customer_old.customers.preferred_billing_day_of_month IS '{"pestroutes_column_name": "preferredBillingDate"}';

COMMENT ON COLUMN customer_old.customers.payment_hold_date IS '{"pestroutes_column_name": "paymentHoldDate"}';

COMMENT ON COLUMN customer_old.customers.most_recent_credit_card_last_four IS '{"pestroutes_column_name": "mostRecentCreditCardLastFour"}';

COMMENT ON COLUMN customer_old.customers.most_recent_credit_card_exp_date IS '{"pestroutes_column_name": "mostRecentCreditCardExpirationDate"}';

COMMENT ON COLUMN customer_old.customers.sms_reminders IS '{"pestroutes_column_name": "smsReminders"}';

COMMENT ON COLUMN customer_old.customers.phone_reminders IS '{"pestroutes_column_name": "phoneReminders"}';

COMMENT ON COLUMN customer_old.customers.email_reminders IS '{"pestroutes_column_name": "emailReminders"}';

COMMENT ON COLUMN customer_old.customers.tax_rate IS '{"pestroutes_column_name": "taxRate"}';

COMMENT ON COLUMN customer_old.customers.billing_company_name IS '{"pestroutes_column_name": "billingCompanyName"}';

COMMENT ON COLUMN customer_old.customers.billing_first_name IS '{"pestroutes_column_name": "billingFName"}';

COMMENT ON COLUMN customer_old.customers.billing_last_name IS '{"pestroutes_column_name": "billingLName"}';

COMMENT ON COLUMN customer_old.customers.billing_country IS '{"pestroutes_column_name": "billingCountryID"}';

COMMENT ON COLUMN customer_old.customers.billing_address IS '{"pestroutes_column_name": "billingAddress"}';

COMMENT ON COLUMN customer_old.customers.billing_city IS '{"pestroutes_column_name": "billingCity"}';

COMMENT ON COLUMN customer_old.customers.billing_state IS '{"pestroutes_column_name": "billingState"}';

COMMENT ON COLUMN customer_old.customers.billing_postal_code IS '{"pestroutes_column_name": "billingZip"}';

COMMENT ON COLUMN customer_old.customers.billing_phone IS '{"pestroutes_column_name": "billingPhone"}';

COMMENT ON COLUMN customer_old.customers.billing_email IS '{"pestroutes_column_name": "billingEmail"}';

COMMENT ON COLUMN customer_old.customers.pestroutes_created_by IS '{"pestroutes_column_name": "addedByID"}';

COMMENT ON COLUMN customer_old.customers.pestroutes_source_id IS '{"pestroutes_column_name": "sourceID"}';

COMMENT ON COLUMN customer_old.customers.pestroutes_preferred_tech_id IS '{"pestroutes_column_name": "preferredTechID"}';

COMMENT ON COLUMN customer_old.customers.pestroutes_customer_link IS '{"pestroutes_column_name": "customerLink"}';

COMMENT ON COLUMN customer_old.customers.pestroutes_created_at IS '{"pestroutes_column_name": "dateAdded"}';

COMMENT ON COLUMN customer_old.customers.pestroutes_cancelled_at IS '{"pestroutes_column_name": "dateCancelled"}';

COMMENT ON COLUMN customer_old.customers.pestroutes_updated_at IS '{"pestroutes_column_name": "dateUpdated"}';

CREATE TABLE customer_old.note_types (
    id integer NOT NULL,
    external_ref_id integer,
    name text NOT NULL,
    pestroutes_office_id integer,
...
ALTER TABLE customer_old.note_types ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME customer_old.note_types_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE customer_old.notes (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    external_ref_id integer,
    area_id integer,
    is_visible_to_customer boolean NOT NULL,
...
CREATE TABLE customer_old.subscriptions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    external_ref_id integer,
    area_id integer,
    customer_id uuid,
...
COMMENT ON COLUMN customer_old.subscriptions.id IS '{"pestroutes_column_name": "subscriptionLink"}';

COMMENT ON COLUMN customer_old.subscriptions.external_ref_id IS '{"pestroutes_column_name": "subscriptionID"}';

COMMENT ON COLUMN customer_old.subscriptions.is_active IS '{"pestroutes_column_name": "active"}';

COMMENT ON COLUMN customer_old.subscriptions.initial_service_quote IS '{"pestroutes_column_name": "initialQuote"}';

COMMENT ON COLUMN customer_old.subscriptions.initial_service_discount IS '{"pestroutes_column_name": "initialDiscount"}';

COMMENT ON COLUMN customer_old.subscriptions.initial_service_total IS '{"pestroutes_column_name": "initialServiceTotal"}';

COMMENT ON COLUMN customer_old.subscriptions.initial_status_id IS '{"pestroutes_column_name": "initialStatus"}';

COMMENT ON COLUMN customer_old.subscriptions.recurring_charge IS '{"pestroutes_column_name": "recurringCharge"}';

COMMENT ON COLUMN customer_old.subscriptions.contract_value IS '{"pestroutes_column_name": "contractValue"}';

COMMENT ON COLUMN customer_old.subscriptions.annual_recurring_value IS '{"pestroutes_column_name": "annualRecurringValue"}';

COMMENT ON COLUMN customer_old.subscriptions.billing_frequency IS '{"pestroutes_column_name": "billingFrequency"}';

COMMENT ON COLUMN customer_old.subscriptions.service_frequency IS '{"pestroutes_column_name": "frequency"}';

COMMENT ON COLUMN customer_old.subscriptions.days_til_follow_up_service IS '{"pestroutes_column_name": "followupService"}';

COMMENT ON COLUMN customer_old.subscriptions.agreement_length_months IS '{"pestroutes_column_name": "agreementLength"}';

COMMENT ON COLUMN customer_old.subscriptions.source IS '{"pestroutes_column_name": "source"}';

COMMENT ON COLUMN customer_old.subscriptions.cancellation_notes IS '{"pestroutes_column_name": "cxlNotes"}';

COMMENT ON COLUMN customer_old.subscriptions.annual_recurring_services IS '{"pestroutes_column_name": "annualRecurringServices"}';

COMMENT ON COLUMN customer_old.subscriptions.renewal_frequency IS '{"pestroutes_column_name": "renewalFrequency"}';

COMMENT ON COLUMN customer_old.subscriptions.max_monthly_charge IS '{"pestroutes_column_name": "maxMonthlyCharge"}';

COMMENT ON COLUMN customer_old.subscriptions.initial_billing_date IS '{"pestroutes_column_name": "initialBillingDate"}';

COMMENT ON COLUMN customer_old.subscriptions.next_service_date IS '{"pestroutes_column_name": "nextService"}';

COMMENT ON COLUMN customer_old.subscriptions.last_service_date IS '{"pestroutes_column_name": "lastCompleted"}';

COMMENT ON COLUMN customer_old.subscriptions.next_billing_date IS '{"pestroutes_column_name": "nextBillingDate"}';

COMMENT ON COLUMN customer_old.subscriptions.expiration_date IS '{"pestroutes_column_name": "expirationDate"}';

COMMENT ON COLUMN customer_old.subscriptions.custom_next_service_date IS '{"pestroutes_column_name": "customDate"}';

COMMENT ON COLUMN customer_old.subscriptions.appt_duration_in_mins IS '{"pestroutes_column_name": "duration"}';

COMMENT ON COLUMN customer_old.subscriptions.preferred_days_of_week IS '{"pestroutes_column_name": "preferredDays"}';

COMMENT ON COLUMN customer_old.subscriptions.preferred_start_time_of_day IS '{"pestroutes_column_name": "preferredStart"}';

COMMENT ON COLUMN customer_old.subscriptions.preferred_end_time_of_day IS '{"pestroutes_column_name": "preferredEnd"}';

COMMENT ON COLUMN customer_old.subscriptions.addons IS '{"pestroutes_column_name": "addOns"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_customer_id IS '{"pestroutes_column_name": "customerID"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_created_by IS '{"pestroutes_column_name": "addedBy"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_sold_by IS '{"pestroutes_column_name": "soldBy"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_sold_by_2 IS '{"pestroutes_column_name": "soldBy2"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_sold_by_3 IS '{"pestroutes_column_name": "soldBy3"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_service_type_id IS '{"pestroutes_column_name": "serviceID"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_source_id IS '{"pestroutes_column_name": "sourceID"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_last_appointment_id IS '{"pestroutes_column_name": "lastAppointment"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_preferred_tech_id IS '{"pestroutes_column_name": "preferredTech"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_initial_appt_id IS '{"pestroutes_column_name": "initialAppointmentID"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_recurring_ticket IS '{"pestroutes_column_name": "recurringTicket"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_subscription_link IS '{"pestroutes_column_name": "subscriptionLink"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_created_at IS '{"pestroutes_column_name": "dateAdded"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_cancelled_at IS '{"pestroutes_column_name": "dateCancelled"}';

COMMENT ON COLUMN customer_old.subscriptions.pestroutes_updated_at IS '{"pestroutes_column_name": "dateUpdated"}';
...
COMMENT ON COLUMN dbe_test.notifications_sent_test.notification_datetime IS '{"field_desc":"When the notification was sent,"IS_PI":true}';

COMMENT ON COLUMN dbe_test.notifications_sent_test.notification_type IS '{"field_desc":"This is a test comment.,"IS_PI":true}';

COMMENT ON COLUMN dbe_test.notifications_sent_test.reference_id IS '{""pestroutes_column_name"": ""appointmentID"","IS_PI":true}';

COMMENT ON COLUMN dbe_test.notifications_sent_test.created_at IS '{""pestroutes_column_name"": ""TestID"","IS_PI":true}';
...
COMMENT ON COLUMN dbe_test.test_comments.test1 IS '{"field_desc":"This is a test1 test,"IS_PI":true}';
...
CREATE TABLE field_operations.office_days_schedule (
    ...
    occurrence character varying(16) DEFAULT NULL::character varying,
    week integer
);
...
CREATE TABLE field_operations_old.appointment_statuses (
    id integer NOT NULL,
    external_ref_id integer,
    name character varying(255) NOT NULL,
    description character varying(255),
...
ALTER TABLE field_operations_old.appointment_statuses ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME field_operations_old.appointment_statuses_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE field_operations_old.appointment_types (
    id integer NOT NULL,
    name character varying(255)
);

ALTER TABLE field_operations_old.appointment_types ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME field_operations_old.appointment_types_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE field_operations_old.appointments (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    external_ref_id integer,
    area_id integer,
    customer_id uuid,
    ...
    status_id integer NOT NULL,
    is_initial boolean NOT NULL,
    tech_time_in_at timestamp with time zone,
    tech_time_out_at timestamp with time zone,
    tech_check_in_at timestamp with time zone,
    tech_check_out_at timestamp with time zone,
    tech_check_in_lat double precision,
    tech_check_in_lng double precision,
    tech_check_out_lat double precision,
    tech_check_out_lng double precision,
...
COMMENT ON COLUMN field_operations_old.appointments.external_ref_id IS '{"pestroutes_column_name": "appointmentID"}';

COMMENT ON COLUMN field_operations_old.appointments.date IS '{"pestroutes_column_name": "date"}';

COMMENT ON COLUMN field_operations_old.appointments.start_time IS '{"pestroutes_column_name": "start"}';

COMMENT ON COLUMN field_operations_old.appointments.end_time IS '{"pestroutes_column_name": "end"}';

COMMENT ON COLUMN field_operations_old.appointments.time_window IS '{"pestroutes_column_name": "timeWindow"}';

COMMENT ON COLUMN field_operations_old.appointments.call_ahead_in_mins IS '{"pestroutes_column_name": "callAhead"}';

COMMENT ON COLUMN field_operations_old.appointments.duration_in_mins IS '{"pestroutes_column_name": "duration"}';

COMMENT ON COLUMN field_operations_old.appointments.is_initial IS '{"pestroutes_column_name": "isInitial"}';

COMMENT ON COLUMN field_operations_old.appointments.tech_time_in_at IS '{"pestroutes_column_name": "timeIn", "timezone_type": "office"}';

COMMENT ON COLUMN field_operations_old.appointments.tech_time_out_at IS '{"pestroutes_column_name": "timeOut", "timezone_type": "office"}';

COMMENT ON COLUMN field_operations_old.appointments.tech_check_in_at IS '{"pestroutes_column_name": "checkIn", "timezone_type": "server"}';

COMMENT ON COLUMN field_operations_old.appointments.tech_check_out_at IS '{"pestroutes_column_name": "checkOut", "timezone_type": "server"}';

COMMENT ON COLUMN field_operations_old.appointments.tech_check_in_lat IS '{"pestroutes_column_name": "latIn"}';

COMMENT ON COLUMN field_operations_old.appointments.tech_check_in_lng IS '{"pestroutes_column_name": "longIn"}';

COMMENT ON COLUMN field_operations_old.appointments.tech_check_out_lat IS '{"pestroutes_column_name": "latOut"}';

COMMENT ON COLUMN field_operations_old.appointments.tech_check_out_lng IS '{"pestroutes_column_name": "longOut"}';

COMMENT ON COLUMN field_operations_old.appointments.notes IS '{"pestroutes_column_name": "notes"}';

COMMENT ON COLUMN field_operations_old.appointments.appt_notes IS '{"pestroutes_column_name": "appointmentNotes"}';

COMMENT ON COLUMN field_operations_old.appointments.office_notes IS '{"pestroutes_column_name": "officeNotes"}';

COMMENT ON COLUMN field_operations_old.appointments.do_interior IS '{"pestroutes_column_name": "doInterior"}';

COMMENT ON COLUMN field_operations_old.appointments.serviced_interior IS '{"pestroutes_column_name": "servicedInterior"}';

COMMENT ON COLUMN field_operations_old.appointments.wind_speed IS '{"pestroutes_column_name": "windSpeed"}';

COMMENT ON COLUMN field_operations_old.appointments.wind_direction IS '{"pestroutes_column_name": "windDirection"}';

COMMENT ON COLUMN field_operations_old.appointments.payment_method IS '{"pestroutes_column_name": "paymentMethod"}';

COMMENT ON COLUMN field_operations_old.appointments.amount_collected IS '{"pestroutes_column_name": "amountCollected"}';

COMMENT ON COLUMN field_operations_old.appointments.temperature IS '{"pestroutes_column_name": "temperature"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_customer_id IS '{"pestroutes_column_name": "customerID"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_subscription_id IS '{"pestroutes_column_name": "subscriptionID"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_service_type_id IS '{"pestroutes_column_name": "type"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_route_id IS '{"pestroutes_column_name": "routeID"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_spot_id IS '{"pestroutes_column_name": "spotID"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_invoice_id IS '{"pestroutes_column_name": "ticketID"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_status_id IS '{"pestroutes_column_name": "status"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_created_by IS '{"pestroutes_column_name": "employeeID"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_assigned_tech IS '{"pestroutes_column_name": "assignedTech"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_serviced_by IS '{"pestroutes_column_name": "servicedBy"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_completed_by IS '{"pestroutes_column_name": "completedBy"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_cancelled_by IS '{"pestroutes_column_name": "cancelledBy"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_created_at IS '{"pestroutes_column_name": "dateAdded", "timezone_type": "server"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_completed_at IS '{"pestroutes_column_name": "dateCompleted", "timezone_type": "server"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_cancelled_at IS '{"pestroutes_column_name": "dateCancelled", "timezone_type": "server"}';

COMMENT ON COLUMN field_operations_old.appointments.pestroutes_updated_at IS '{"pestroutes_column_name": "dateUpdated", "timezone_type": "server"}';

CREATE TABLE field_operations_old.route_groups (
    id integer NOT NULL,
    name character varying(255) NOT NULL,
    created_at timestamp(6) with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at timestamp(6) with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
ALTER TABLE field_operations_old.route_groups ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME field_operations_old.route_groups_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE field_operations_old.routes (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    external_ref_id integer,
    area_id integer NOT NULL,
    name character varying(255),
...
CREATE TABLE organization_old.dealers (
    id integer NOT NULL,
    name character varying(255) NOT NULL,
    created_by uuid,
    updated_by uuid,
...
ALTER TABLE organization_old.dealers ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME organization_old.dealers_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE organization_old.user_dealers (
    user_id uuid NOT NULL,
    dealer_id integer NOT NULL,
    created_by uuid,
    updated_by uuid,
...
CREATE TABLE organization_old.users (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE pestroutes.service_types (
    id integer NOT NULL,
    area_id integer,
    name text NOT NULL,
    created_at timestamp(6) with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
COMMENT ON COLUMN pestroutes.service_types.id IS '{"pestroutes_column_name": "typeID"}';

COMMENT ON COLUMN pestroutes.service_types.name IS '{"pestroutes_column_name": "description"}';

CREATE VIEW pestroutes.sync_to_pestroutes_queue AS
 SELECT 'audit'::text AS schema_name,
    'customer__customers'::text AS table_name,
    customer__customers.id,
    customer__customers.table_id,
    customer__customers.operation,
    customer__customers.row_old,
    customer__customers.row_new,
    customer__customers.row_diff,
    customer__customers.db_user_name,
    customer__customers.db_user_ip,
    customer__customers.created_at,
    customer__customers.pestroutes_sync_status,
    customer__customers.pestroutes_synced_at,
    customer__customers.lambda_request_ids
   FROM audit.customer__customers
  WHERE ((customer__customers.pestroutes_sync_status = 'PENDING'::text) AND (customer__customers.db_user_name <> 'lambda_pestroutes_etl'::text));
...
CREATE TABLE product_old.plans (
    id integer NOT NULL,
    external_ref_id integer,
    name text
);

ALTER TABLE product_old.plans ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME product_old.plans_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE ONLY audit.customer__customers
    ADD CONSTRAINT customer__customers_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY auth_old.actions
    ADD CONSTRAINT actions_name_key UNIQUE (name);

ALTER TABLE ONLY auth_old.actions
    ADD CONSTRAINT actions_pkey PRIMARY KEY (id);

ALTER TABLE ONLY auth_old.fields
    ADD CONSTRAINT fields_name_key UNIQUE (name);

ALTER TABLE ONLY auth_old.fields
    ADD CONSTRAINT fields_pkey PRIMARY KEY (id);

ALTER TABLE ONLY auth_old.idp_roles
    ADD CONSTRAINT idp_roles_pkey PRIMARY KEY (id);

ALTER TABLE ONLY auth_old.permissions
    ADD CONSTRAINT permissions_pkey PRIMARY KEY (id);

ALTER TABLE ONLY auth_old.resources
    ADD CONSTRAINT resources_name_key UNIQUE (name);

ALTER TABLE ONLY auth_old.resources
    ADD CONSTRAINT resources_pkey PRIMARY KEY (id);

ALTER TABLE ONLY auth_old.role_permissions
    ADD CONSTRAINT role_permissions_pkey PRIMARY KEY (id);

ALTER TABLE ONLY auth_old.role_permissions
    ADD CONSTRAINT role_permissions_role_id_permission_id_unique UNIQUE (role_id, permission_id);

ALTER TABLE ONLY auth_old.roles
    ADD CONSTRAINT roles_pk UNIQUE (name);

ALTER TABLE ONLY auth_old.roles
    ADD CONSTRAINT roles_pkey PRIMARY KEY (id);

ALTER TABLE ONLY auth_old.services
    ADD CONSTRAINT services_name_key UNIQUE (name);

ALTER TABLE ONLY auth_old.services
    ADD CONSTRAINT services_pkey PRIMARY KEY (id);

ALTER TABLE ONLY auth_old.user_roles
    ADD CONSTRAINT user_roles_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY billing_old.invoices
    ADD CONSTRAINT invoices_external_ref_id_key UNIQUE (external_ref_id);

ALTER TABLE ONLY billing_old.invoices
    ADD CONSTRAINT invoices_pkey PRIMARY KEY (id);

ALTER TABLE ONLY billing_old.payment_gateways
    ADD CONSTRAINT payment_gateways_pkey PRIMARY KEY (id);

ALTER TABLE ONLY billing_old.payment_invoice_allocations
    ADD CONSTRAINT payment_invoice_applications_payment_id_invoice_id_key UNIQUE (payment_id, invoice_id);

ALTER TABLE ONLY billing_old.payment_invoice_allocations
    ADD CONSTRAINT payment_invoice_applications_pkey PRIMARY KEY (id);

ALTER TABLE ONLY billing_old.payment_methods
    ADD CONSTRAINT payment_methods_external_ref_id_key UNIQUE (external_ref_id);

ALTER TABLE ONLY billing_old.payment_methods
    ADD CONSTRAINT payment_methods_pkey PRIMARY KEY (id);

ALTER TABLE ONLY billing_old.payment_statuses
    ADD CONSTRAINT payment_statuses_pkey PRIMARY KEY (id);

ALTER TABLE ONLY billing_old.payment_types
    ADD CONSTRAINT payment_types_pkey PRIMARY KEY (id);

ALTER TABLE ONLY billing_old.payment_types
    ADD CONSTRAINT payment_types_sort_order_key UNIQUE (sort_order);

ALTER TABLE ONLY billing_old.payments
    ADD CONSTRAINT payments_external_ref_id_key UNIQUE (external_ref_id);
...
ALTER TABLE ONLY customer_old.cancellation_reasons
    ADD CONSTRAINT cancellation_reasons_external_ref_id_key UNIQUE (external_ref_id);

ALTER TABLE ONLY customer_old.cancellation_reasons
    ADD CONSTRAINT cancellation_reasons_pkey PRIMARY KEY (id);

ALTER TABLE ONLY customer_old.customers
    ADD CONSTRAINT customers_external_ref_id_key UNIQUE (external_ref_id);

ALTER TABLE ONLY customer_old.customers
    ADD CONSTRAINT customers_pkey PRIMARY KEY (id);

ALTER TABLE ONLY customer_old.note_types
    ADD CONSTRAINT note_types_external_ref_id_key UNIQUE (external_ref_id);

ALTER TABLE ONLY customer_old.note_types
    ADD CONSTRAINT note_types_pkey PRIMARY KEY (id);

ALTER TABLE ONLY customer_old.notes
    ADD CONSTRAINT notes_external_ref_id_key UNIQUE (external_ref_id);

ALTER TABLE ONLY customer_old.notes
    ADD CONSTRAINT notes_pkey PRIMARY KEY (id);

ALTER TABLE ONLY customer_old.subscriptions
    ADD CONSTRAINT subscriptions_external_ref_id_key UNIQUE (external_ref_id);
...
ALTER TABLE ONLY field_operations_old.appointment_statuses
    ADD CONSTRAINT appointment_statuses_external_ref_id_key UNIQUE (external_ref_id);

ALTER TABLE ONLY field_operations_old.appointment_statuses
    ADD CONSTRAINT appointment_statuses_pkey PRIMARY KEY (id);

ALTER TABLE ONLY field_operations_old.appointment_types
    ADD CONSTRAINT appointment_types_pkey PRIMARY KEY (id);

ALTER TABLE ONLY field_operations_old.appointments
    ADD CONSTRAINT appointments_external_ref_id_key UNIQUE (external_ref_id);

ALTER TABLE ONLY field_operations_old.appointments
    ADD CONSTRAINT appointments_pkey PRIMARY KEY (id);

ALTER TABLE ONLY field_operations_old.route_groups
    ADD CONSTRAINT route_groups_name_key UNIQUE (name);

ALTER TABLE ONLY field_operations_old.route_groups
    ADD CONSTRAINT route_groups_pkey PRIMARY KEY (id);

ALTER TABLE ONLY field_operations_old.routes
    ADD CONSTRAINT routes_external_ref_id_key UNIQUE (external_ref_id);
...
ALTER TABLE ONLY organization_old.dealers
    ADD CONSTRAINT dealers_name_key UNIQUE (name);

ALTER TABLE ONLY organization_old.dealers
    ADD CONSTRAINT dealers_pkey PRIMARY KEY (id);

ALTER TABLE ONLY organization_old.user_dealers
    ADD CONSTRAINT user_dealers_pkey PRIMARY KEY (user_id, dealer_id);

ALTER TABLE ONLY organization_old.users
    ADD CONSTRAINT users_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY pestroutes.service_types
...
ALTER TABLE ONLY product_old.plans
    ADD CONSTRAINT plans_pkey PRIMARY KEY (id);
...
CREATE INDEX customer__customers_created_at ON audit.customer__customers USING btree (created_at);

CREATE INDEX customer__customers_db_user ON audit.customer__customers USING btree (db_user_name);

CREATE INDEX customer__customers_table_id ON audit.customer__customers USING btree (table_id);

CREATE INDEX customer__subscriptions_db_user ON audit.customer__subscriptions USING btree (db_user_name);

CREATE INDEX customer__subscriptions_table_id ON audit.customer__subscriptions USING btree (table_id);
...
CREATE INDEX idx_transactions_payment_type_id ON billing.transactions USING btree (payment_type_id);
...
CREATE UNIQUE INDEX payment_methods_customer_id_is_primary_true ON billing_old.payment_methods USING btree (customer_id, is_primary) WHERE ((deleted_at IS NULL) AND (is_primary = true));
...
CREATE UNIQUE INDEX user_external_ref_id ON organization_old.users USING btree (external_ref_id) WHERE (deleted_at IS NULL);

CREATE INDEX idx_employees_user_id ON pestroutes.employees USING btree (user_id);

CREATE INDEX idx_service_types_appointment_type_id ON pestroutes.service_types USING btree (appointment_type_id);

CREATE INDEX idx_service_types_plan_id ON pestroutes.service_types USING btree (plan_id);

CREATE UNIQUE INDEX teams_name_dealer_id_idx ON sales.teams USING btree (name, dealer_id);

CREATE INDEX cells_area_resolution ON spt.cells_old USING btree (area_id, resolution);

CREATE INDEX cells_boundary ON spt.cells_old USING gist (boundary);
...
CREATE TRIGGER customers_after_insert_or_update_or_delete AFTER INSERT OR DELETE OR UPDATE ON customer_old.customers FOR EACH ROW EXECUTE FUNCTION public.create_audit_record();

CREATE TRIGGER subscriptions_after_insert_or_update_or_delete AFTER INSERT OR DELETE OR UPDATE ON customer_old.subscriptions FOR EACH ROW EXECUTE FUNCTION public.create_audit_record();
...
CREATE TRIGGER appointments_before_insert BEFORE UPDATE ON field_operations_old.appointments FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_v2();
...
ALTER TABLE ONLY auth.permissions
    ADD CONSTRAINT permissions_action_id_fkey FOREIGN KEY (action_id) REFERENCES auth.actions(id);

ALTER TABLE ONLY auth.permissions
    ADD CONSTRAINT permissions_field_id_fkey FOREIGN KEY (field_id) REFERENCES auth.fields(id);

ALTER TABLE ONLY auth.permissions
    ADD CONSTRAINT permissions_resource_id_fkey FOREIGN KEY (resource_id) REFERENCES auth.resources(id);

ALTER TABLE ONLY auth.permissions
    ADD CONSTRAINT permissions_service_id_fkey FOREIGN KEY (service_id) REFERENCES auth.services(id);
...
ALTER TABLE ONLY auth.user_roles
    ADD CONSTRAINT user_roles_user_id_fkey FOREIGN KEY (user_id) REFERENCES organization.users(id);

ALTER TABLE ONLY auth_old.idp_roles
    ADD CONSTRAINT idp_roles_roles__fk FOREIGN KEY (role_id) REFERENCES auth_old.roles(id);

ALTER TABLE ONLY auth_old.permissions
    ADD CONSTRAINT permissions_action_id_fkey FOREIGN KEY (action_id) REFERENCES auth_old.actions(id);

ALTER TABLE ONLY auth_old.permissions
    ADD CONSTRAINT permissions_field_id_fkey FOREIGN KEY (field_id) REFERENCES auth_old.fields(id);

ALTER TABLE ONLY auth_old.permissions
    ADD CONSTRAINT permissions_resource_id_fkey FOREIGN KEY (resource_id) REFERENCES auth_old.resources(id);

ALTER TABLE ONLY auth_old.permissions
    ADD CONSTRAINT permissions_service_id_fkey FOREIGN KEY (service_id) REFERENCES auth_old.services(id);

ALTER TABLE ONLY auth_old.role_permissions
    ADD CONSTRAINT role_permissions_permission_id_fkey FOREIGN KEY (permission_id) REFERENCES auth_old.permissions(id);

ALTER TABLE ONLY auth_old.role_permissions
    ADD CONSTRAINT role_permissions_role_id_fkey FOREIGN KEY (role_id) REFERENCES auth_old.roles(id);

ALTER TABLE ONLY auth_old.user_roles
    ADD CONSTRAINT user_roles_role_id_fkey FOREIGN KEY (role_id) REFERENCES auth_old.roles(id);

ALTER TABLE ONLY auth_old.user_roles
    ADD CONSTRAINT user_roles_user_id_fkey FOREIGN KEY (user_id) REFERENCES organization_old.users(id);
...
ALTER TABLE ONLY billing.transaction_types
    ADD CONSTRAINT transaction_types_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing.transaction_types
    ADD CONSTRAINT transaction_types_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing.transactions
    ADD CONSTRAINT transactions_type_id_foreign FOREIGN KEY (payment_type_id) REFERENCES billing_old.payment_types(id) ON DELETE CASCADE;

ALTER TABLE ONLY billing_old.invoices
    ADD CONSTRAINT invoices_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.invoices
    ADD CONSTRAINT invoices_customer_id_fkey FOREIGN KEY (customer_id) REFERENCES customer_old.customers(id);

ALTER TABLE ONLY billing_old.invoices
    ADD CONSTRAINT invoices_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.invoices
    ADD CONSTRAINT invoices_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_gateways
    ADD CONSTRAINT payment_gateways_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_gateways
    ADD CONSTRAINT payment_gateways_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_gateways
    ADD CONSTRAINT payment_gateways_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_invoice_allocations
    ADD CONSTRAINT payment_invoice_applications_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_invoice_allocations
    ADD CONSTRAINT payment_invoice_applications_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_invoice_allocations
    ADD CONSTRAINT payment_invoice_applications_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_methods
    ADD CONSTRAINT payment_methods_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_methods
    ADD CONSTRAINT payment_methods_customer_id_fkey FOREIGN KEY (customer_id) REFERENCES customer_old.customers(id);

ALTER TABLE ONLY billing_old.payment_methods
    ADD CONSTRAINT payment_methods_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_methods
    ADD CONSTRAINT payment_methods_payment_gateway_id_fkey FOREIGN KEY (payment_gateway_id) REFERENCES billing_old.payment_gateways(id);

ALTER TABLE ONLY billing_old.payment_methods
    ADD CONSTRAINT payment_methods_payment_type_id_fkey FOREIGN KEY (payment_type_id) REFERENCES billing_old.payment_types(id);

ALTER TABLE ONLY billing_old.payment_methods
    ADD CONSTRAINT payment_methods_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_statuses
    ADD CONSTRAINT payment_statuses_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_statuses
    ADD CONSTRAINT payment_statuses_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_statuses
    ADD CONSTRAINT payment_statuses_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_types
    ADD CONSTRAINT payment_types_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_types
    ADD CONSTRAINT payment_types_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payment_types
    ADD CONSTRAINT payment_types_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payments
    ADD CONSTRAINT payments_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payments
    ADD CONSTRAINT payments_customer_id_fkey FOREIGN KEY (customer_id) REFERENCES customer_old.customers(id);

ALTER TABLE ONLY billing_old.payments
    ADD CONSTRAINT payments_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY billing_old.payments
    ADD CONSTRAINT payments_payment_gateway_id_fkey FOREIGN KEY (payment_gateway_id) REFERENCES billing_old.payment_gateways(id);

ALTER TABLE ONLY billing_old.payments
    ADD CONSTRAINT payments_payment_status_id_fkey FOREIGN KEY (payment_status_id) REFERENCES billing_old.payment_statuses(id);

ALTER TABLE ONLY billing_old.payments
    ADD CONSTRAINT payments_payment_type_id_fkey FOREIGN KEY (payment_type_id) REFERENCES billing_old.payment_types(id);

ALTER TABLE ONLY billing_old.payments
    ADD CONSTRAINT payments_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);
...
ALTER TABLE ONLY customer_old.customers
    ADD CONSTRAINT customers_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY customer_old.customers
    ADD CONSTRAINT customers_dealer_id_fkey FOREIGN KEY (dealer_id) REFERENCES organization_old.dealers(id);

ALTER TABLE ONLY customer_old.customers
    ADD CONSTRAINT customers_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY customer_old.customers
    ADD CONSTRAINT customers_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY customer_old.notes
    ADD CONSTRAINT notes_cancellation_reason_id_fkey FOREIGN KEY (cancellation_reason_id) REFERENCES customer_old.cancellation_reasons(id);

ALTER TABLE ONLY customer_old.notes
    ADD CONSTRAINT notes_customer_id_fkey FOREIGN KEY (customer_id) REFERENCES customer_old.customers(id);

ALTER TABLE ONLY customer_old.notes
    ADD CONSTRAINT notes_note_type_id_fkey FOREIGN KEY (note_type_id) REFERENCES customer_old.note_types(id);

ALTER TABLE ONLY customer_old.subscriptions
    ADD CONSTRAINT subscriptions_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY customer_old.subscriptions
    ADD CONSTRAINT subscriptions_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY customer_old.subscriptions
    ADD CONSTRAINT subscriptions_sold_by_2_fkey FOREIGN KEY (sold_by_2) REFERENCES organization_old.users(id);

ALTER TABLE ONLY customer_old.subscriptions
    ADD CONSTRAINT subscriptions_sold_by_3_fkey FOREIGN KEY (sold_by_3) REFERENCES organization_old.users(id);

ALTER TABLE ONLY customer_old.subscriptions
    ADD CONSTRAINT subscriptions_sold_by_fkey FOREIGN KEY (sold_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY customer_old.subscriptions
    ADD CONSTRAINT subscriptions_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);
...
ALTER TABLE ONLY field_operations.routes
    ADD CONSTRAINT routes_user_assigned_to_fkey FOREIGN KEY (user_assigned_to) REFERENCES organization.users(id);

ALTER TABLE ONLY field_operations.service_types
    ADD CONSTRAINT service_types_new_plan_id_fkey FOREIGN KEY (plan_id) REFERENCES product.plans(id);

ALTER TABLE ONLY field_operations_old.appointments
    ADD CONSTRAINT appointments_appointment_type_id_fkey FOREIGN KEY (appointment_type_id) REFERENCES field_operations_old.appointment_types(id);

ALTER TABLE ONLY field_operations_old.appointments
    ADD CONSTRAINT appointments_customer_id_fkey FOREIGN KEY (customer_id) REFERENCES customer_old.customers(id);

ALTER TABLE ONLY field_operations_old.appointments
    ADD CONSTRAINT appointments_status_id_fkey FOREIGN KEY (status_id) REFERENCES field_operations_old.appointment_statuses(id);

ALTER TABLE ONLY field_operations_old.appointments
    ADD CONSTRAINT appointments_subscription_id_fkey FOREIGN KEY (subscription_id) REFERENCES customer_old.subscriptions(id);

ALTER TABLE ONLY field_operations_old.routes
    ADD CONSTRAINT routes_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY field_operations_old.routes
    ADD CONSTRAINT routes_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY field_operations_old.routes
    ADD CONSTRAINT routes_route_group_id_fkey FOREIGN KEY (route_group_id) REFERENCES field_operations_old.route_groups(id);

ALTER TABLE ONLY field_operations_old.routes
    ADD CONSTRAINT routes_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY field_operations_old.routes
    ADD CONSTRAINT routes_user_assigned_to_fkey FOREIGN KEY (user_assigned_to) REFERENCES organization_old.users(id);
...
ALTER TABLE ONLY organization.user_dealers
    ADD CONSTRAINT user_dealers_user_id_fkey FOREIGN KEY (user_id) REFERENCES organization.users(id);

ALTER TABLE ONLY organization_old.dealers
    ADD CONSTRAINT dealers_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY organization_old.dealers
    ADD CONSTRAINT dealers_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY organization_old.dealers
    ADD CONSTRAINT dealers_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY organization_old.user_dealers
    ADD CONSTRAINT user_dealers_created_by_fkey FOREIGN KEY (created_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY organization_old.user_dealers
    ADD CONSTRAINT user_dealers_dealer_id_fkey FOREIGN KEY (dealer_id) REFERENCES organization_old.dealers(id);

ALTER TABLE ONLY organization_old.user_dealers
    ADD CONSTRAINT user_dealers_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES organization_old.users(id);

ALTER TABLE ONLY organization_old.user_dealers
    ADD CONSTRAINT user_dealers_user_id_fkey FOREIGN KEY (user_id) REFERENCES organization_old.users(id);

ALTER TABLE ONLY pestroutes.employees
    ADD CONSTRAINT employees_user_id_fkey FOREIGN KEY (user_id) REFERENCES organization_old.users(id);

ALTER TABLE ONLY pestroutes.etl_execution_steps
    ADD CONSTRAINT etl_execution_steps_execution_id_fkey FOREIGN KEY (execution_id) REFERENCES pestroutes.etl_executions(id);

ALTER TABLE ONLY pestroutes.service_types
    ADD CONSTRAINT service_types_appointment_type_id_fkey FOREIGN KEY (appointment_type_id) REFERENCES field_operations_old.appointment_types(id);

ALTER TABLE ONLY pestroutes.service_types
    ADD CONSTRAINT service_types_plan_id_fkey FOREIGN KEY (plan_id) REFERENCES product_old.plans(id);
...
ALTER TABLE ONLY sales.user_teams
    ADD CONSTRAINT user_teams_user_id_fkey FOREIGN KEY (user_id) REFERENCES organization.users(id);

CREATE PUBLICATION airbyte_publication WITH (publish = 'insert, update, delete, truncate');

ALTER PUBLICATION airbyte_publication ADD TABLE ONLY audit.customer__customers;
