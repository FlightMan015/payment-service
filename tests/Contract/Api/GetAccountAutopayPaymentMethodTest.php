...
class GetAccountAutopayPaymentMethodTest extends AbstractContractTest
{
    private const string ENDPOINT_URI = '/api/v1/accounts/%s/autopay-payment-method';

    #[Test]
    public function it_returns_200_response_for_retrieving_account_autopay_payment_method_successfully(): void
    {
        $accountId = Str::uuid()->toString();
        $paymentMethod = $this->createPaymentMethodInDatabase(accountId: $accountId, isAutopay: true);
    ...
    {
        $response = $this->sendRequest(accountId: Str::uuid(), headers: ['Api-Key' => 'invalid']);
    ...
    #[Test]
    public function it_returns_404_not_found_response_when_autopay_payment_method_is_not_found(): void
    {
        ...
        $response->assertValid();
        $response->assertJsonPath(path: '_metadata.success', expect: false);
        $response->assertJsonPath(path: 'result.message', expect: __('messages.account.autopay_payment_method_not_found'));
    }
    ...
    private function createPaymentMethodInDatabase(string $accountId, bool $isAutopay): PaymentMethod
    {
        $account = Account::factory()->create(attributes: ['id' => $accountId]);
        $paymentMethod = PaymentMethod::factory()->for($account)->create();
        if ($isAutopay) {
            $account->setAutoPayPaymentMethod(autopayPaymentMethod: $paymentMethod);
        }
