...
class CheckStatusTest extends UnitTestCase
{
    use GatewayMockingTrait;
    use ValidatorMockingTrait;
    use RepositoryMockingTrait;

    private CheckStatus $operation;

    protected function setUp(): void
    {
        parent::setUp();

        $this->operation = OperationStub::checkStatus();
    }

    #[Test]
    public function process_method_sets_raw_request_and_calls_gateway_status_method(): void
    {
        $request = [
        ...
        $gateway->expects($this->once())->method('status')->with($request);
    ...
    {
        $this->operation->setValidator(validator: $this->mockSuccessValidator(className: CheckStatusValidator::class));

        $this->assertInstanceOf(expected: CheckStatus::class, actual: $this->operation->validate());
    ...
    {
        $this->operation->setValidator(validator: $this->mockFailValidator(className: CheckStatusValidator::class));
    ...
    public function process_method_throws_invalid_operation_exception_when_gateway_throws_invalid_operation_exception(): void
    {
        ...
        $gateway->method('status')->willThrowException($exception);
        $this->operation->setGateway(gateway: $gateway);
