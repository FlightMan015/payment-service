...
class CollectPaymentSettledMetricsListenerTest extends UnitTestCase
{
    private CollectPaymentSettledMetricsListener $listener;
    ...
    #[Test]
    public function it_is_listening_to_the_payment_settled_event(): void
    {
        Event::assertListening(
            expectedEvent: PaymentSettledEvent::class,
            expectedListener: CollectPaymentSettledMetricsListener::class
        );
    }
    ...
    public function it_logs_start_process_and_stores_metric_in_influx(): void
    {
        ...
        Log::shouldReceive('info')->once()->with(
            'Collect Payment Settled Metrics Job Started',
            ['job_id' => $this->listener->job?->uuid()]
        );

        $expectedPoint = new Point('payments-settled');
        $expectedPoint
    ...
    public function it_logs_an_error_when_an_exception_is_thrown(): void
    {
        ...
        Log::shouldReceive('error')->once()->with(
            'FAILED - Collect Payment Settled Metrics',
            [
        ...
        $this->mockWriteApi->shouldReceive('close')->once();
    ...
    private function setupListener(): void
    {
        ...
        $this->listener = new CollectPaymentSettledMetricsListener(
            influxClient: $this->mockInfluxClient,
        );
    }

    private function setupEvent(): PaymentSettledEvent
    {
        ...
        $transaction = Transaction::factory()->withoutRelationships()->make(
            attributes: [
                ...
                'raw_response_log' => json_encode([
                    0,
                    WorldpayResponseStub::statusSuccess(isSettled: true),
                ]),
            ]
        );
        ...
        return new PaymentSettledEvent(payment: $payment);
    }
