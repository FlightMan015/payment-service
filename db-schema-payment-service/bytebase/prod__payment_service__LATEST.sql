...
CREATE TABLE `tmp_payment_methods_initial_load` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `company_id` BIGINT UNSIGNED NOT NULL,
  `customer_id` BIGINT UNSIGNED NOT NULL,
  `office_id` SMALLINT UNSIGNED DEFAULT NULL COMMENT 'PestRoutes office id',
...
CREATE UNIQUE INDEX `unique_ext_reference_id_per_company` ON `tmp_payment_methods_initial_load` (`ext_reference_id`, `company_id`);

CREATE INDEX `payment_methods_gateway_id_foreign` ON `tmp_payment_methods_initial_load` (`gateway_id`);

CREATE INDEX `payment_methods_payment_type_id_foreign` ON `tmp_payment_methods_initial_load` (`payment_type_id`);
